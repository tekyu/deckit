...
io.on("connection", socket => {
        ...
        findServer(data.room)
            .then(room => {
                ...
                if (
                    ...
                    // if is emit winning player
                    // else change round to next
                    // // reset picked to false
                    // // room.hinter = getNextHinter(room)
                    // // reset player.pickedCard, player.roundCard, room.pickedCards, room.roundCards
                    // // change stage to 'hintable'
                    // then updateRoom
                } else {
                    updateRoom(room.id, room);
                }
    ...
    const updatePlayer = (players, player) => {
        return players.map(_player => {
            return player.id === _player.id ? _player : player;
        });
    };
