...
console.log("PROC", process.env.DEVDB_CONN);
...
const getCardsFromServer = () =>
  ...
  request('http://deckit.patrykrosiak.pl/cards.json', (
			error,
			response,
			body
		) => {
  ...
  try {
			if (!room || room === waitingRoom) {
				const connectedPlayers = io.sockets.adapter.rooms[
					waitingRoom
				].playersConnected.filter((player) => player.id !== id);
...
io.on("connection", socket => {
      ...
      .then((res) => {
        ...
        if (!id || id === waitingRoom) {
          io.in(waitingRoom).emit(
            'playersInWaitingRoom',
            res.playersConnected,
          console.log(chalk.bgBlue("[emitting] playersInWaitingRoom"));
        } else if (res.playersConnected.length === 0) {
  ...
  socket.on("changePlayerStatusInRoom", params => {
    ...
    findServer(params.room).then((res) => {
      const _player = res.playersConnected.filter((player) => player.id === socket.id)[0];
      _player.status = params.status;
      const newReadyStatus =				!(res.playersConnected.filter((player) => !player.status).length > 0);
   ...
   */
  const checkForDuplicate = (randomCard, deck) => (deck.filter(function(card) {
   ...
   */
  const distributeCards = (room) => new Promise((resolve, reject) => {
  ...
  socket.on("sendHint", data => {
    ...
    findServer(data.room).then((room) => {
      ...
      player.deck = player.deck.filter((card) => card.id !== data.selectedCard.id);
      room.playersConnected = room.playersConnected.map((_player) => (_player.id === player.id ? player : _player));
  ...
  socket.on("sendPickedCard", data => {
    ...
    findServer(data.room).then((room) => {
      const player = room.playersConnected.filter((_player) => _player.id === socket.id)[0];

      player.pickedCard = data.pickedCard;
      player.picked = true;

      player.deck = player.deck.filter(card => card.id !== data.pickedCard.id);
      room.playersConnected = room.playersConnected.map((_player) => (_player.id === player.id ? player : _player));
      if (
        room.stage === 'pickable'
    ...
    findServer(data.room)
      .then((room) => {
        ...
        room.playersConnected = room.playersConnected.map(_player =>
          _player.id === player.id ? player : _player
        );
        ...
        if (
          ...
          room.winners = room.playersConnected.filter(
            player => player.score >= room.maxPoints
          );
  ...
  const distributeCards2 = (room) => {
    room.playersConnected = room.playersConnected.map((player) => {
      while (player.deck.length < room.deckSize) {
        ...
        const { _randomCard, _randomIndex } = getRandomCard(room.initialCards);
   ...
   */
  const getPickedCards = (room) => room.playersConnected.map((player) => ({
				id: player.id,
				card:
   ...
   * @returns {Object} Object of picked cards
   */
  const getRoundCards = (room) => room.playersConnected.reduce((total, player) => {
   ...
   */
  const hasAllPlayersPicked = (players, hinter) => (players.filter(player => {
   ...
   */
  const movePickedToMine = (players) => players.map((player) => ({
				...player,
   ...
   * @returns {Object} Object of connected players
   */
  const resetPlayersPicked = (players, mod) => players.map((player) => ({
				...player,
				picked: false,
				pickedCard: null,
  ...
  const calculateRoundPoints = (room) => {
    ...
    // if all players find the hinter card
    const cardsEqualToHinterCard = room.roundCards.filter((card) => card.card === hintedCard.id);
    ...
    if (cardsEqualToHinterCard.length === room.roundCards.length) {
      ...
      cardsEqualToHinterCard.forEach((card) => {
        hinterPlayer.score += 3;
        const ownerOfTheCard = findPlayerByCardId(
          _tempRoom.playersConnected,
          card.card,
        );
        ownerOfTheCard.score += 3;
  ...
  const updatePlayer = (players, player) => {
    const newPlayers = players.map((_player) => (player.id === _player.id ? player : _player));
    return newPlayers;
  };
  ...
  const findPlayerByCardId = (players, cardId) => {
    const foundPlayer = players.filter((player) => player.pickedCard === cardId);
  ...
  const findPlayer = (players, id) => {
    const _player = players.filter((player) => player.id === id);
